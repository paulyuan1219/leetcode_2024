...
I keep modifying the code, but it's still not right
```python
class Solution:
    def continuousSubarrays(self, nums: List[int]) -> int:
        n = len(nums)
...
check this array, my solution detect 2 windows as following, Apparently, there are overlaps
- [65,66,67,66,66,65]
- [66,66,65,64,65,65,64]

we can use a set to store each interval, but this will exceed the memory... to bad
...
class Solution:
    ...
    def updateSet(self, start, end, ht):
        for i in range(start, end+1):
            for j in range(i, end+1):
                ht.add((i, j))

```
It's too difficult. Stop here.
